...
from pylocogym.data.deep_mimic_pretrained import (
    DeepMimicPretrained,
    DeepMimicMotionDataFieldNames,
)
...
class ResidualEnv(PylocoEnv):
    def __init__(self, max_episode_steps, env_params, reward_params, enable_rand_init=True):
        ...
        #Dataloader for Pretrained Data
        self.pretrained_motion = DeepMimicPretrained(reward_params["pretrained_clips_file_path"], len(self.motion)+1)
        self.pretrained_loop = LoopKeyframeMotionDataset(
            self.pretrained_motion, num_loop=self.clips_repeat_num, track_fields=[BobMotionDataFieldNames.ROOT_POS]
        )
        self.pretrained_lerp = LerpMotionDataset(
            self.pretrained_loop,
            lerp_fields=[
                DeepMimicMotionDataFieldNames.ROOT_POS,
            ],
            alerp_fields=[
        ...
        self.pretrained_adapter = DeepMimicMotionBobAdapter(
            reward_params["pretrained_clips_file_path"],
            self.num_joints,
            self.joint_angle_limit_low,
            self.joint_angle_limit_high,
            self.joint_angle_default,
    ...
    def step(self, action: List[np.ndarray]):
        ...
        res = self.pretrained_lerp.eval(next_t)
